...
- [-] **19. SMS 通知** (2026-06-12 廃案)  
  Twilio 等での SMS 完了通知案。**user 判断で構想完全に外す**:<br>
  (a) 注文ごとに電話番号入力 = ゲスト UX 摩擦が大きい (mocal は「アプリ不要・気軽なテイクアウト」が prop)。<br>
  (b) 1 通 ¥5-10 のランニングコスト + Twilio account 維持コスト。<br>
  (c) 個人情報追加収集の負担。<br>
  代替: email 通知 + realtime page で対応。iOS PWA install への誘導は別途検討 (#21 候補)。
- [ ] **20. マーケティング自動化**  
