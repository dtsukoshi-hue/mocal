...
import {
  ...
  updateWaitMinutesAction,
  clearStoreOverrideAction,
...
export default function StoreToggle({
  ...
  async function doToggle() {
    ...
    // 手動 override が設定される（actions/store.ts の toggleStoreOpenAction 内で当日 23:59:59 まで設定）
    const jstMidnight = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Tokyo' }).format(new Date())
    setOverrideUntil(new Date(`${jstMidnight}T23:59:59+09:00`).toISOString())
    startTransition(() => router.refresh())
  }

  async function doChangeWait(min: number) {
    const prev = waitMinutes
    setWaitMinutes(min)
    closeSheet()

    const result = await updateWaitMinutesAction(min)
    if (result?.error) {
      setWaitMinutes(prev)
      setErrorMessage(result.error)
      return
  ...
  async function clearOverride() {
    ...
    const result = await clearStoreOverrideAction()
    if (result?.error) {
        ...
        >
          <span
            className={`w-2 h-2 rounded-full shrink-0 ${
              optimistic ? 'bg-white/60 animate-pulse' : 'bg-stone-500'
            }`}
          />
